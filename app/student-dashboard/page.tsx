...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    interactionApi
      .getChatPartners(user.email)
      .then((r) => setChatPartners(r.data || []));
  ...
  useEffect(() => {
    if (!user) return;
    interactionApi
      .getStudentMentorships(user.email)
      .then((r) => setMentorships(r.data || []));
  }, [user, tab]);
  ...
  const sendMessage = async () => {
    ...
    setSending(true);
    await interactionApi.sendMessage(
      user.email,
      user.name,
      activeChat,
      msgInput,
    ...
    if (!chatPartners.includes(activeChat))
      setChatPartners((p) => [...p, activeChat]);
  };

  const requestMentorship = async (alumniEmail: string) => {
    if (!user || !mentorMsg.trim()) {
      showToast("Please enter a message", "error");
      return;
    }
    const r = await interactionApi.sendMentorshipRequest(
      user.email,
      alumniEmail,
      mentorMsg,
    );
  ...
  const filteredAlumni = alumniList.filter((a) => {
    const matchSearch =
      !search ||
      a.sName?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
        ...
        <div className="p-5">
          ...
          <div className="bg-slate-800 rounded-lg p-3 mb-6">
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 ${getBg(user?.email || "")} rounded-xl flex items-center justify-center text-lg`}
              >
      ...
      <div className="flex-1 ml-60">
        ...
        {toast && (
          <div
            className={`fixed top-5 right-5 z-50 flex items-center gap-3 px-5 py-3.5 rounded-lg shadow-2xl text-sm font-medium transition-all ${
              toast.type === "success"
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  return (
                    <div
                      key={c.label}
                      className={`bg-gradient-to-br ${c.color} rounded-lg p-5 text-white`}
                    >
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                  ...
                  {mentorships.length === 0 ? (
                    ...
                    mentorships.slice(0, 3).map((m: any) => (
                      ...
                      >
                        <div
                          className={`w-9 h-9 ${getBg(m.alumniEmail)} rounded-xl flex items-center justify-center text-sm`}
                        >
            ...
            <div className="space-y-5">
              <div>
                ...
                <p className="text-gray-500 text-sm mt-1">
                  Connect with {alumniList.length} alumni professionals
                </p>
              </div>
                  ...
                  filteredAlumni.map((a: any) => (
                    ...
                    >
                      <div className="flex items-start gap-3 mb-4">
                        <div
                          className={`w-12 h-12 ${getBg(a.email)} rounded-2xl flex items-center justify-center text-xl shrink-0`}
                        >
                          {getAvatar(a.email)}
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1.5">
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full font-medium ${BRANCH_COLORS[a.branch] || "bg-gray-100 text-gray-600"}`}
                            >
                              {a.branch}
                            </span>
                            <span className="text-xs text-gray-400 flex items-center gap-1">
                              <Calendar className="w-3 h-3" /> {a.sGradYear}
                            </span>
                          </div>
                        </div>
                      </div>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                  <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
                    ...
                    <div className="flex items-center gap-4 mb-5">
                      <div
                        className={`w-16 h-16 ${getBg(viewProfile.email)} rounded-2xl flex items-center justify-center text-3xl shrink-0`}
                      >
                      ...
                      <div>
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-medium mt-1 inline-block ${BRANCH_COLORS[viewProfile.branch] || "bg-gray-100 text-gray-600"}`}
                        >
                          {viewProfile.branch} · Class of{" "}
                          {viewProfile.sGradYear}
                        </span>
                      </div>
                    </div>
                    <div className="space-y-3 bg-gray-50 rounded-xl p-4 mb-4">
                      {viewProfile.sCurrPos && (
                    ...
                    <div className="flex gap-2">
                      {viewProfile.sLinkedInUrl && (
                        <a
                          href={viewProfile.sLinkedInUrl}
                          target="_blank"
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                  <div className="bg-white rounded-3xl shadow-2xl p-6 w-full max-w-sm animate-in fade-in zoom-in duration-200">
                    ...
                    <div className="flex items-center gap-3 bg-blue-50 rounded-lg p-3 mb-4">
                      <div
                        className={`w-11 h-11 ${getBg(selectedAlumni.email)} rounded-xl flex items-center justify-center text-xl`}
                      >
                        {getAvatar(selectedAlumni.email)}
                      </div>
                      <div>
                        <p className="font-semibold text-gray-900">
                          {selectedAlumni.sName}
                        </p>
                        <p className="text-xs text-gray-500">
                          {selectedAlumni.branch} · Class of{" "}
                          {selectedAlumni.sGradYear}
                        </p>
                      </div>
                    </div>
                    <textarea
                    ...
                    />
                    <button
                      onClick={() => requestMentorship(selectedAlumni.email)}
            ...
            <div className="space-y-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-500 text-sm mt-1">
                    {mentorships.length} total requests
                  </p>
                </div>
                <button
                  onClick={() => setTab("alumni")}
              ...
              {mentorships.length === 0 ? (
                <div className="bg-white rounded-3xl border border-gray-100 p-16 text-center shadow-sm">
                  ...
                  <p className="text-gray-400 text-sm mb-5">
                    Connect with alumni to get guidance on your career
                  </p>
                  <button
                    onClick={() => setTab("alumni")}
                  ...
                  {mentorships.map((m: any) => (
                    ...
                    >
                      <div className="flex items-start gap-4">
                        <div
                          className={`w-12 h-12 ${getBg(m.alumniEmail)} rounded-2xl flex items-center justify-center text-xl shrink-0`}
                        >
                          {getAvatar(m.alumniEmail)}
                        </div>
                        <div className="flex-1 min-w-0">
              ...
              <div className="w-72 bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col">
                <div className="p-4 border-b border-gray-50">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {chatPartners.length} conversations
                  </p>
                </div>
                <div className="flex-1 overflow-y-auto p-2 space-y-1">
                  {chatPartners.length === 0 ? (
                    ...
                    chatPartners.map((p) => (
                      ...
                      >
                        <div
                          className={`w-10 h-10 ${getBg(p)} rounded-xl flex items-center justify-center text-lg shrink-0`}
                        >
                          {getAvatar(p)}
                        </div>
                        <div className="flex-1 min-w-0">
                ...
                {!activeChat ? (
                  <div className="flex-1 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-sm text-gray-400">
                        Select a chat or find an alumni to message
                      </p>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="p-4 border-b border-gray-50 flex items-center gap-3">
                      <div
                        className={`w-10 h-10 ${getBg(activeChat)} rounded-xl flex items-center justify-center text-lg`}
                      >
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50/50">
                      ...
                      {messages.map((m: any) => (
                        <div
                          key={m.id}
                          className={`flex ${m.senderEmail === user?.email ? "justify-end" : "justify-start"}`}
                        >
                          <div
                            className={`max-w-xs lg:max-w-sm ${m.senderEmail === user?.email ? "items-end" : "items-start"} flex flex-col gap-1`}
                          >
                            <div
                  ...
                  {jobs.map((j: any) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between">
                          <p className="text-xs text-gray-400">
                            {j.applicants?.length || 0} applicant(s)
                          </p>
                          <button
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={async () => {
                            if (!user) return;
                            const r = await interactionApi.registerForEvent(
                              e.id,
                              user.email,
                              user.name,
                              "STUDENT",
            ...
            <div className="space-y-5 max-w-lg">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                <div className="flex items-center gap-5 mb-6">
                  <div
                    className={`w-20 h-20 ${getBg(user?.email || "")} rounded-2xl flex items-center justify-center text-4xl shadow-lg`}
                  >
                    {getAvatar(user?.email || "")}
                  </div>
                  <div>
                    ...
                    <span className="mt-1.5 inline-block text-xs bg-blue-100 text-blue-700 px-3 py-1 rounded-full font-semibold">
                      Student
                    </span>
                  </div>
                </div>
                <div className="space-y-1">
                  {[
                    ...
                    {
                      label: "Mentorships",
                      value: `${mentorships.length} requests`,
                    },
                    {
                      label: "Jobs Applied",
                      value: `${jobs.filter((j: any) => j.applicants?.includes(user?.email)).length}`,
                    },
                  ].map((f) => (
                    <div
                      key={f.label}
                  ...
                  <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
                    ...
                    <div className="space-y-3">
                      ...
                      <button
                        onClick={async () => {
                          if (!user) return;
                          const r = await interactionApi.updateProfile(
                            user.email,
                            profileForm,
                          );
                          if (r.success) {
